...
class SEPCFCOS(nn.Module):
    ...
    def forward(self, images, features, gt_instances):
        ...
        if self.export_onnx:
            return logits_pred, reg_pred, ctrness_pred
...
class FCOSHead(nn.Module):
    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(sepc_outs):
            # feature = self.share_tower(feature)
            cls_tower = self.cls_tower(feature[0])
            bbox_tower = self.bbox_tower(feature[1])

...
class FashionEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if "classification" in output:
                classification = output["classification"].to(self._cpu_device)
                prediction["classification"] = classification_to_json(classification, input["image_id"])
    ...
    def evaluate(self):
        ...
        if self._output_dir:
            ...
            file_path = os.path.join(self._output_dir, "fashion_predictions.pth")
        ...
        if "classification" in self._predictions[0]:
            self._eval_classification()
        ...
        for task in sorted(tasks):
            coco_eval = (
                _evaluate_predictions_on_coco(
                    self._fashion_api, self._coco_results, task, kpt_oks_sigmas=self._kpt_oks_sigmas
                )
    ...
    def _eval_classification(self):
        ...
        Fill self._results with the metrics of the tasks.
        """
        self._logger.info("Preparing results for Fashion format ...")
        # TODO: the classification is not a list
        #self._classification_results = list(itertools.chain(*[x["classification"] for x in self._predictions]))
        self._classification_results = [x["classification"] for x in self._predictions]

        # unmap the category ids for Fashion
        if hasattr(self._metadata, "classification_dataset_id_to_contiguous_id"):
            reverse_id_mapping = {
                v: k for k, v in self._metadata.classification_dataset_id_to_contiguous_id.items()
            }
            for result in self._classification_results:
                category_id = result["category_id"]
                assert (
                        category_id in reverse_id_mapping
                ), "A classification has category_id={}, which is not available in the dataset.".format(
                    category_id
                )
                result["category_id"] = reverse_id_mapping[category_id]

        # unmap part and toward to label
        for result in self._classification_results:
            result["toward"] += 1
            result["part"] += 1

        if self._output_dir:
            file_path = os.path.join(self._output_dir, "fashion_classification_results.json")
            self._logger.info("Saving results to {}".format(file_path))
            with PathManager.open(file_path, "w") as f:
                f.write(json.dumps(self._classification_results))
                f.flush()

        if not self._do_evaluation:
            self._logger.info("Annotations are not available for classification evaluation.")
            return
...
def classification_to_json(classification, img_id):
    """
    Args:
        classification: the output of the model
        img_id (str): the image id in Dataset

    Returns:
        dict: the results in fashionnet evaluation format
    """

    # only one classification for one picture
    result = {
        "image_id": img_id,
        "category_id": classification.category_id.item(),
        "part": classification.part.item(),
        "toward": classification.toward.item(),
        "category_score": classification.category_score.item(),
        "part_score": classification.part_score.item(),
        "toward_score": classification.toward_score.item(),
    }

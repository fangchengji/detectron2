...
class RepeatFactorTrainingSampler(Sampler):
    ...
    def __init__(self, dataset_dicts, repeat_thresh, shuffle=True, seed=None):
        """
        Args:
            dataset_dicts (list[dict]): annotations in Detectron2 data format.
    ...
    def _get_repeat_factors(self, dataset_dicts, repeat_thresh):
        ...
        Returns:
            torch.Tensor: the i-th element is the repeat factor for the data image
                at index i.
        """
        # 1. For each category c, compute the fraction of images that contain it: f(c)
        category_freq = defaultdict(int)
        for dataset_dict in dataset_dicts:  # For each image (without repeats)
            cat_ids = {ann["category2_id"] for ann in dataset_dict["annotations2"]}
    ...
    def _get_epoch_indices(self, generator):
        """
        Create a list of data indices (with repeats) to use for one epoch.
        ...
        Returns:
            torch.Tensor: list of data indices to use in one epoch. Each index
